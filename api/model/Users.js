...
class Users {
  fetchUsers(req, res) {
    const query = `
    SELECT userID, userName, lastName, emailAdd, gender, age FROM users
    `;
  ...
  fetchUser(req, res) {
    const query = `
    SELECT userID, userName, lastName, emailAdd,gender, age FROM users WHERE userID = ${req.params.id}
    `;
